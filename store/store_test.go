...
func Test_GetByID(t *testing.T) {
	...
	testCases := []struct {
		...
		input       int
		dbMock      []interface{}
		expOutput   *model.Employee
		expectedErr error
	}{
		{desc: "success case", input: 1,
			dbMock: []interface{}{mock.ExpectQuery(getByIDQuery).
				WillReturnRows(sqlmock.NewRows([]string{"id", "name", "dept"}).AddRow(1,
					"SAMPLE-NAME", "tech"))},
			expOutput:   &model.Employee{ID: 1, Name: "SAMPLE-NAME", Dept: "tech"},
			expectedErr: nil},
		...
		{desc: "failure case", input: 1,
			...
			expectedErr: errors.EntityNotFound{Entity: "employee", ID: "1"}},
	}
...
func Test_Update(t *testing.T) {
	...
	testCases := []struct {
		...
		{desc: "failure case", input: &model.Employee{ID: 1, Name: "sample-name"},
			dbMock: []interface{}{mock.ExpectExec(updateQuery).
				WillReturnError(errors.Error("error from db"))},
			expOutput:   nil,
			expectedErr: errors.DB{Err: errors.Error("error from db")}},
		{desc: "success case", input: &model.Employee{ID: 1, Name: "sample-name"},
			dbMock: []interface{}{mock.ExpectExec(updateQuery).
				...
				mock.ExpectQuery(updateQuery).
					WillReturnRows(sqlmock.NewRows([]string{"id", "domain"}).AddRow("f0ddfd00-8554-4ccc-b6cf-eb7577b5dbbb",
						"auth.zopsmart.com"))},
			expOutput:   &model.Employee{ID: 1, Name: "sample-name"},
			expectedErr: nil},
	}
